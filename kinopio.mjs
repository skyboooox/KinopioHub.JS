...
export async function client(opt) {
   ...
   * @returns 
   */
  const request_raw = async (endpoint = [options.zoneID, nanoid()], data = {}, timeout = 3) => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const m = await nats.request(endpoint.join("."), JSON.stringify(data), { timeout: timeout * 1000 });
   ...
   * @returns 
   */
  const request = async (endpoint = [nanoid()], data = {}, timeout = 3) => {
    return new Promise(async (resolve, reject) => {
