...
export class ConnectionManager {
  ...
  async #establish(plan, { previousConnection = null, timeoutMs, expectedServer } = {}) {
    ...
    try {
      assertCurrent();
      // Mark dirty BEFORE rebinding: a partial rebind failure has already
      // moved some wires off the previous connection and needs the rollback.
      rebound = true;
      ...
      // rebindAll moved the wire subscriptions onto the failed candidate; put
      // them back on the still-active previous connection so a failed switch
      // does not leave live subscriptions dead.
      if (
        rebound &&
        previousConnection &&
        !this.#disposed &&
        ...
        try {
          await this.#rebindAll(previousConnection);
        } catch (rebindError) {
          this.#log("warn", "Failed to rebind subscriptions to the previous connection after an aborted switch:", rebindError);
        }
      }
      throw error;
    }
  ...
  async #runHealthCheck(connection, epoch) {
    try {
      ...
      // status() completing means the connection closed for good (e.g.
      // reconnect attempts exhausted). Reach a terminal state so
      // connected() waiters are not left pending against a dead connection.
      if (
        !this.#disposed &&
        epoch === this.#epoch &&
        this.#healthCheckActive &&
        ...
        this.#log("error", "NATS connection closed permanently");
  ...
  waitForConnected(timeoutMs = DEFAULT_CONNECTED_WAIT_TIMEOUT_MS) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (timeoutMs !== null && timeoutMs !== undefined) {
        // Kept ref'd: this timer is what a caller awaiting connected() is
        // blocked on — unref'ing it can let the process exit with the await
        // unsettled.
        timer = setTimeout(() => {
          cleanup();
  ...
  async #cleanup() {
    // Bump the epoch FIRST so an in-flight #establish aborts at its next
    // checkpoint, then serialize behind the switch lock so a candidate that
    // already passed its checks cannot be promoted after the teardown.
    this.#epoch += 1;
