...
export class DiscoveryProber {
  ...
  async runProbeCycle({ forceRefresh = false, reschedule = true } = {}) {
    ...
    const cyclePromise = (async () => {
      try {
        ...
        return await this.#applyManifest(manifest, discoveryOptions);
      } catch (error) {
        ...
        if (reschedule && discoveryOptions.backgroundLocalProbe && this.#manager.connection && !this.#stopped) {
          this.scheduleProbeCycle(discoveryOptions.cacheTtlMs);
        }
      }
    })().finally(() => {
  ...
  async #switchToRemoteFallback(discoveryOptions, reason) {
  ...
  async #applyManifest(manifest, discoveryOptions) {
    const currentPlan = this.#manager.activePlan;
    const currentServer = this.#manager.connection?.getServer?.() ?? currentPlan?.connectedServer ?? null;

    if (!manifest) {
      return await this.#switchToRemoteFallback(discoveryOptions, "no usable local leader manifest");
    }

    const localServer = manifest.websocketUrl || manifest.wssUrl;
    if (!localServer) {
      return await this.#switchToRemoteFallback(discoveryOptions, "manifest did not expose a usable websocketUrl");
    }
