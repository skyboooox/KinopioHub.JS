...
function decodeName(buffer, offset, visited = new Set()) {
  if (visited.has(offset)) {
    throw new Error("mDNS name compression loop detected");
  }
...
export function extractKinopioLeafManifests(packet, {
  ...
  for (const entry of serviceInstances.values()) {
    ...
    if (!namespace || !nodeId || !Number.isInteger(websocketPort)) {
      continue;
    }
