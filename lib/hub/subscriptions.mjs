...
/**
 * Max callback invocations queued per handle before newer messages are
 * dropped for that handle (slow-consumer protection; mirrors the NATS
 * client's own slow-consumer semantics instead of retaining unbounded
 * message references behind a hung callback).
 */
const MAX_PENDING_CALLBACKS_PER_HANDLE = 1_024;
...
export class SubscriberHandle {
  ...
  dispatch(data, message, onError) {
    ...
    if (this.#pendingCallbacks >= MAX_PENDING_CALLBACKS_PER_HANDLE) {
      onError?.(new Error(
        `Subscriber callback backlog exceeded ${MAX_PENDING_CALLBACKS_PER_HANDLE} pending messages; dropping message for this handle`,
      ));
      return;
    }
...
export class SubscriptionRegistry {
  ...
  setService(subject, handler, options, connection) {
    ...
    // Bind-then-commit: the new wire subscription must be live before the
    // previous service is stopped, so a bind failure leaves the existing
    // service untouched. (Both share the same queue group, so the brief
    // overlap splits requests between two working handlers, never drops them.)
    if (connection) {
